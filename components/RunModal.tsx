  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="fixed inset-0 bg-gray-500/75 dark:bg-black/75 transition-opacity" aria-hidden="true" onClick={onClose}></div>
        <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

        <div className="relative inline-block align-bottom bg-white dark:bg-slate-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
            ...
            <div className="sm:flex sm:items-start">
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={onClose} className="text-slate-400 hover:text-slate-500 dark:text-slate-500 dark:hover:text-slate-400">
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="w-full lg:w-1/3 flex flex-col gap-4">
                    ...
                    {variables.length > 0 && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-900/50">
                        <p className="text-xs font-bold text-blue-800 dark:text-blue-300 uppercase tracking-wide mb-3">Variables</p>
                        <div className="space-y-3">
                          {variables.map(variable => (
                            <div key={variable}>
                              <label className="block text-xs font-medium text-blue-900 dark:text-blue-200 mb-1">{variable}</label>
                              <input
                                type="text"
                                value={inputs[variable]}
                                onChange={(e) => handleInputChange(variable, e.target.value)}
                                className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-md p-2 border"
                     ...
                     <div className="flex-grow flex flex-col">
                        <div className="flex justify-between items-center mb-2">
                           <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Prompt Preview</label>
                           <button 
                             onClick={optimizePrompt}
                             disabled={optimizing || !!result}
                             className={`text-xs flex items-center gap-1 px-2 py-1 rounded-md border transition-colors ${
                               optimizing 
                                 ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-400 dark:text-indigo-300 cursor-wait' 
                                 : 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'
                             }`}
                        ...
                        <textarea
                          ...
                          className="w-full flex-grow min-h-[200px] rounded-md border-gray-300 dark:border-slate-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-3 text-sm font-mono text-slate-600 dark:text-slate-300 dark:bg-slate-800 leading-relaxed resize-none"
                        />
                  ...
                  {/* Right Column: Output */}
                  <div className="w-full lg:w-2/3 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800 lg:pl-6 pt-6 lg:pt-0 min-h-[400px] flex flex-col">
                    <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wide mb-3">AI Output</h3>
                    
                    <div className="flex-grow bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 relative overflow-hidden flex flex-col">
                      ...
                      {loading && (
                       ...
                       {/* Error State */}
                      {error && (
                         <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 text-sm border-b border-red-100 dark:border-red-900/50">
                       ...
                       <div>
                         {result && (
                            <button
                             ...
                             className="text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400 font-medium flex items-center gap-1"
                            >
                       ...
                       <div className="flex gap-3">
                         {result ? (
                             <button
                               ...
                               className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-md hover:bg-slate-50 dark:hover:bg-slate-700 text-sm font-medium transition-colors"
                             >
                               Clear
                             </button>
                         ) : null}
